...
int VideoStreamer::start() {
    ...
    pthread_create(&busyLoopThread, NULL, busyLoop, (void*) this);
...
void* busyLoop (void* arg) {
    ...
    cv::VideoCapture cap;
    bool openedSuccessfully = cap.open(0);
    ...
    //vector<int> args(_args, _args + sizeof(_args) / sizeof(int));
    ...
    while(true) {
        ...
        cv::imencode(".bmp", frame, jpgBuffer);
        
        //int vidDataLen = cvMatDataToBuff(&frame, vidDataBuff); 
        
        int vidDataLen = cvImageDataToBuff(jpgBuffer, vidDataBuff);
        
        int bytesWritten = newVidDataPacket(sendBuff, vidDataBuff, vidDataLen, i, frameSize.width, frameSize.height);
